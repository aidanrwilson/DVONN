...
public:
    ...
    bool is_connected()
    {
        return line_network->is_connected();
    }
    // the following functions return 0 or "" if not connected
    std::string get_remote_host()
    {
        return line_network->get_remote_host();
    }
    int get_remote_port()
    {
        return line_network->get_remote_port();
    }
    std::string get_local_host()
    {
        return line_network->get_local_host();
    }
    int get_local_port()
    {
        return line_network->get_local_port();
    }

    void set_handler( Message_Network_Handler<Message> *handler );
    void set_auto_flush( bool auto_flush )
    {
        line_network->set_auto_flush(auto_flush);
    }
    Line_Network *get_line_network()
    {
        return line_network;
    }
    wxSocketBase *get_socket()
    {
        return line_network->get_socket();
    }
protected:
...
public:
    ...
    int get_port()
    {
        return line_server->get_port();    // returns 0 if not bound
    }

    wxSocketBase *get_socket()
    {
        return line_server->get_socket();
    }
protected:
